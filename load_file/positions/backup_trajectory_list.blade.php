            ...
            <div class="col-sm-12">
                <div class="tabs-container">
                    <ul class="nav nav-tabs">
                        ...
                        <li class="active"><a href="{{route('admin.positions.backup_trajectory_list')}}">已归还的轨迹列表</a></li>
                    ...
                    <form role="form" class="form-inline form-screen" method="get">
                        ...
                        <div class="form-group">
                            ...
                            <select name="dlj_type">
                                ...
                                <option @if(request('dlj_type')==1) selected @endif value="1">十代机</option>
                                <option @if(request('dlj_type')==2) selected @endif value="2">欧7</option>
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <button type="button" class="btn btn-white" onclick="location.href='{{route('admin.positions.backup_trajectory_list')}}'">重置</button>
                    </form>
                </div>
            </div>
        </div>
                    ...
                    <table class="table table-striped table-new table-hover dataTables-example dataTable">
                        ...
                        @foreach($info as $g)
                            <tr class="gradeA">
                                ...
                                <td>@if($g->dlj_type==1)十代机 @else 欧7 @endif</td>
                                <td>{{$g->RENT_STARTTIME}}</td>
                                <td>{{$g->back_time}}</td>
                                <td>{{$g->RENT_NAME}}</td>
                                <td>{{$g->RENT_CARDNO}}</td>
                                <td>
                                    <a href="{{route('admin.positions.backup_trajectory_info',[$g->RENT_ID])}}">查看轨迹</a>
                                </td>
                            </tr>
                        @endforeach
                    </table>
