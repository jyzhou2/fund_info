...
define('echarts/echarts', [
    ...
    Echarts.prototype = {
        ...
        _onclick: function (param) {
            ...
            if (param.target) {
//              px = param.event.layerX;
//              py = param.event.layerY;
				/*IE下的layerX、layerY值有偏差，改为取offsetX、offsetY值*/
                px = param.event.offsetX;
                py = param.event.offsetY;
        ...
        _eventPackage: function (target) {
            if (target) {
                ...
                var x = (px - target.position[0]) / target.scale[0];
                var y = (py - target.position[1]) / target.scale[0];
                return {
